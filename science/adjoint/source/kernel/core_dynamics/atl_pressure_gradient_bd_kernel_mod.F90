...
!-----------------------------------------------------------------------------
!> @brief Computes the adjoint for the boundary integral part of
!>        rhs of the momentum equation for the nonlinear equations.
!>
module atl_pressure_gradient_bd_kernel_mod

  use argument_mod, only : adjacent_face, arg_type, &
                           cell_column, cross, &
                           func_type, gh_basis, &
                           gh_diff_basis, gh_field, &
                           gh_inc, gh_quadrature_face, &
                           gh_read, gh_readwrite, &
                           gh_real, gh_scalar, &
                           mesh_data_type, outward_normals_to_horizontal_faces, &
  ...
  type, public, extends(kernel_type) :: atl_pressure_gradient_bd_kernel_type

    private

    type(arg_type) :: meta_args(9) = (/                                    &
         arg_type(GH_FIELD, GH_REAL, GH_READ, W2, STENCIL(CROSS)),         & ! r_u_bd
         arg_type(GH_FIELD, GH_REAL, GH_READWRITE, W3),                    & ! exner
         arg_type(GH_FIELD, GH_REAL, GH_READ, W3, STENCIL(CROSS)),         & ! dummy_w3
         arg_type(GH_FIELD, GH_REAL, GH_READWRITE, Wtheta),                & ! theta
         arg_type(GH_FIELD * 3, GH_REAL, GH_READWRITE, Wtheta),            & ! moist_dyn
         arg_type(GH_FIELD, GH_REAL, GH_READ, W3, STENCIL(CROSS)),         & ! ls_exner
         arg_type(GH_FIELD, GH_REAL, GH_READ, Wtheta, STENCIL(CROSS)),     & ! ls_theta
         arg_type(GH_FIELD * 3, GH_REAL, GH_READ, Wtheta, STENCIL(CROSS)), & ! ls_moist_dyn
  ...
  contains
    procedure, nopass :: atl_pressure_gradient_bd_code
  end type
  ...
  !-------------------------------------------------------------------------------
  public :: atl_pressure_gradient_bd_code

  contains
  !> @brief Adjoint for tl_pressure_gradient_bd_code.
  !> @details Handwritten adjoint for tl_pressure_gradient_bd_code.
  !> Stencil use results in non owner-compute generated code from PSyAD,
  !> so handwritten solution is necessary for efficiency.
  !> @param[in]     nlayers              Number of layers
  !> @param[in,out] r_u_bd               ACTIVE Change (increment) in RHS of the momentum equation
  !> @param[in]     stencil_w2_size      Size of the W2 stencil (number of cells)
  !> @param[in]     stencil_w2_map       W2 dofmaps for the stencil
  !> @param[in]     exner                ACTIVE Change (increment) in Exner pressure
  !> @param[in]     dummy_w3             Dummy field to get needed stencil information
  !> @param[in]     stencil_w3_size      Size of the W3 stencil (number of cells)
  !> @param[in]     stencil_w3_map       W3 dofmaps for the stencil
  !> @param[in]     theta                ACTIVE Change (increment) in Potential temperature
  !> @param[in]     moist_dyn_gas        ACTIVE Change (increment) in Gas factor
  !> @param[in]     moist_dyn_tot        ACTIVE Change (increment) in Total mass factor
  !> @param[in]     moist_dyn_fac        ACTIVE Change (increment) in Water factor
  ...
  !> @param[in]     stencil_ignore_size  Dummy stencil size argument for PSyclone to work
  !> @param[in]     stencil_ignore_map   Dummy stencil map argument for PSyclone to work
  ...
  !>
  subroutine atl_pressure_gradient_bd_code( nlayers, &
                                            r_u_bd, &
                                            stencil_w2_size, &
                                            stencil_w2_map, &
                                            exner, &
    ...
    integer(kind=i_def), intent(in) :: stencil_ignore_size
    ...
    real(kind=r_def),    dimension(undf_w3), intent(in) :: dummy_w3
    ...
    real(kind=r_def), dimension(ndf_w2) :: pressure_gradient_bd_e_remote
    ...
    real(kind=r_def) :: ls_theta_v_at_fquad_remote
    integer :: i
      ...
      do face = nfaces_re_h, 1, -1
        ...
        do qp = nqp_f, 1, -1
          ...
          do df = ndf_w2, 1, -1
            ...
            res_dot_product_remote = 0.0_r_def
            do i = 1, 3, 1
            ...
            exner_av = exner_av - cp * res_dot_product * ls_theta_v_at_fquad * bdary_term
            exner_av_remote = exner_av_remote - cp * res_dot_product_remote * ls_theta_v_at_fquad_remote * bdary_term_remote
  ...
  end subroutine atl_pressure_gradient_bd_code

end module atl_pressure_gradient_bd_kernel_mod
